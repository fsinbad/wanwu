...
│                                      应用层                                          │
│                                                                                     │
│    examples/backend-wga                    examples/backend-wga-sandbox             │
│    ┌───────────────────────┐              ┌───────────────────────────┐            │
│    │ HandleSSE()           │              │ HandleSSE()               │            │
...
│  │   ├─ CopyToSandbox(local, dest)复制文件到沙箱                                  │ │
│  │   ├─ CopyFromSandbox(local)    复制文件到本地                                  │ │
│  │   └─ Cleanup(ctx)              清理工作目录                                    │ │
│  │                                                                               │ │
│  │  实现模式                                                                      │ │
...
│  │   │   ├─ createRequirementFile()创建 .requirement.md                          │ │
│  │   │   ├─ 复制 skills 到 .opencode/skills/                                    │ │
│  │   │   ├─ 转换 tools 为 skills (openapi-to-skills)                             │ │
│  │   │   └─ 复制 input 目录内容                                                 │ │
│  │   │                                                                           │ │
│  │   ├─ Run(ctx) → chan string                                                  │ │
│  │   │   └─ 执行: opencode run --format json [--thinking] "prompt"              │ │
│  │   │                                                                           │ │
│  │   └─ AfterRun(ctx)                                                           │ │
...
────────────────────
  ...
  │                                                                       │
  │  ┌────────────────┐   ┌─────────────────┐   ┌────────────────────┐  │
  │  │wga_sandbox.Run │──▶│ opencode runner │──▶│ JSON 输出流        │  │
  │  └────────────────┘   └─────────────────┘   └──────────┬─────────┘  │
  │                                                        │            │
  │  ┌─────────────────────────────────────────────────────▼──────────┐ │
...
────────────────────────────
  ...
  │                                                                       │
  │  执行: opencode run --format json "prompt"                             │
  │                                                                       │
  │  输出流:                                                               │
  ...
  └───────────────────────────────────────────────────────────────────────┘
              ...
              ▼
  ┌───────────────────────────────────────────────────────────────────────┐
  │ manager.Cleanup(runID) (除非 SkipCleanup=true)                         │
  │  └─ 删除沙箱工作目录                                                   │
  └───────────────────────────────────────────────────────────────────────┘
              │
              ▼
...
```
    ...
    依赖说明:
     ...
     ├─ wga-sandbox 不依赖 ag-ui-util (输出 JSON 格式，由调用方选择是否转换)
     └─ ag-ui-util 完全独立 (可单独使用)

    使用场景:
     ├─ examples/backend-wga: wga.Run() → EinoTranslator → AG-UI
     └─ examples/backend-wga-sandbox: wga_sandbox.Run() → OpencodeTranslator → AG-UI
```
...
agent.yaml  ──────────────────▶   config.Agent
  ...
  configure:
    max_iterations: 10               .Configure
    enable_thinking: true  ─────────▶ .EnableThinking ──────▶  EnableThinking
  tool_categories:
    - category: search               .ToolCategories
      tools:
        - relative_path: tool.json       .Tools[].Doc
